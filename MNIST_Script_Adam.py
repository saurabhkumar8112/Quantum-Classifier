...
'''def callbackF(Xi):
    ...
    accuracy=info[1]*len(x_train_0)+info[2]*len(x_train_1)
    accuracy/=(len(x_train_0)+len(x_train_1))
    ...
    print("Accuracy : "+str(accuracy[0][0])) # [0][0] since its returning a 2D array
    losses.append(info[0][0])
    accuracies.append(accuracy[0][0])
    losses_t.append(info_t[0][0])
    accuracies_t.append(accuracy_t[0][0])'''
...
for t in tqdm.tqdm(range(1,1000)):
    ...
    beta_2 = 0.999
    epsilon= 1e-8
    g = grad(param,x_train_0[np.random.choice(len(x_train_0), 128)],x_train_1[np.random.choice(len(x_train_0), 128)])
    #print(g)
...
with open("data_"+str(class_name)+"adam.pickle",'wb') as pickle_file:
...
plt.savefig("Loss_"+str(class_name)+"adam.png")
plt.show()
